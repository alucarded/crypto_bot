...
public:
  ...
  IndicatorsGenerator() : m_ob_imbalance(), m_last_ts(0),
      m_sell_weighted_price_sum(0), m_buy_weighted_price_sum(0), m_market_sell_vol(0), m_market_buy_vol(0), m_mid_price(0) {

  }

  virtual ~IndicatorsGenerator() {
     std::ofstream results_file("indicators.csv", std::ios::out | std::ios::app | std::ios::binary);
     results_file << "Weighted_Sell_Trade_Price,Weighted_Buy_Trade_Price,Mid_Price,Imbalance_0.001,Imbalance_0.01,Imbalance_0.1,Buy_Volumes,Sell_Volumes\n";
     assert(m_imbalances_1.size() == m_sell_trade_prices.size());
     for (size_t i = 0; i < m_sell_trade_prices.size(); ++i) {
       results_file << m_sell_trade_prices[i] << "," << m_buy_trade_prices[i] << "," << m_mid_prices[i] << "," << m_imbalances_01[i] << "," << m_imbalances_1[i] << "," << m_imbalances_10[i] << "," << m_buy_volumes[i] << "," << m_sell_volumes[i] << "\n";
     }
     results_file.close();
  }

  virtual void OnConnectionOpen(const std::string&) override {

  }

  virtual void OnConnectionClose(const std::string&) override {
  ...
  virtual void OnTradeTicker(const TradeTicker& ticker) override {
    ...
    assert(ticker.arrived_ts >= m_last_ts);
    assert(m_imbalances_01.size() == m_buy_trade_prices.size());
    assert(m_imbalances_1.size() == m_buy_trade_prices.size());
    assert(m_imbalances_10.size() == m_buy_trade_prices.size());
    if (ticker.is_market_maker) {
      m_market_sell_vol += ticker.qty;
      m_sell_weighted_price_sum += ticker.price * ticker.qty;
    } else {
      m_market_buy_vol += ticker.qty;
      m_buy_weighted_price_sum += ticker.price * ticker.qty;
    }
    m_last_ts = ticker.arrived_ts;
  }
  ...
  virtual void OnOrderBookUpdate(const OrderBook& order_book) override {
    ...
    assert(m_imbalances_01.size() == m_sell_trade_prices.size());
    assert(m_imbalances_1.size() == m_sell_trade_prices.size());
    assert(m_imbalances_10.size() == m_sell_trade_prices.size());
    m_sell_trade_prices.push_back(m_sell_weighted_price_sum/(m_market_sell_vol + m_market_buy_vol));
    m_buy_trade_prices.push_back(m_buy_weighted_price_sum/(m_market_sell_vol + m_market_buy_vol));
    ...
    m_sell_weighted_price_sum = 0;
    m_buy_weighted_price_sum = 0;
    m_market_sell_vol = 0;
    m_market_buy_vol = 0;
...
private:
  ...
  uint64_t m_last_ts;
  double m_sell_weighted_price_sum;
  double m_buy_weighted_price_sum;
  double m_market_sell_vol;
  double m_market_buy_vol;
  double m_mid_price;
  std::vector<double> m_sell_trade_prices;
  std::vector<double> m_buy_trade_prices;
