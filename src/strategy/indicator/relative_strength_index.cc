...
std::optional<double> RelativeStrengthIndex::Calculate(const std::vector<double>& close_values) const {
  ...
  if (sz <= m_period) {
    BOOST_LOG_TRIVIAL(warning) << "[RelativeStrengthIndex::Calculate] Insufficient data: has " << sz << " periods, requires " << m_period + 1;
    return std::nullopt;
  }
  double u, d, smma_u, smma_d;
  CalcuateChanges(close_values[0], close_values[1], u, d);
  smma_u = u;
  smma_d = d;
